...
get_var_df <- function(vcffile){
    ...
    fpfn_vcf <- vcf[rownames(vcf) %in% tt_class_df$variant]
    
    ## Variant type
    fpfn_df <- geno(fpfn_vcf)[['BVT']] %>%
        as.data.frame() %>%
        rownames_to_column(var = "variant") %>%
        dplyr::rename(QUERY.Var = QUERY, TRUTH.Var = TRUTH) %>%
        left_join(tt_class_df)
    
    ## Filter
    fpfn_filter <- rowRanges(fpfn_vcf)$FILTER
    
    ## Stratifications
    fpfn_regions <- info(fpfn_vcf)[['Regions']] %>% as.list()
    
    ## Combining into single data frame
    fpfn_df %>%
        add_column(FILTER = fpfn_filter,
                   Regions = fpfn_regions)
...
annotate_var_df <-  function(var_df){
    ...
    var_anno_df %>% 
        mutate(var_cat = case_when( QUERY == "FP" & QUERY.Var == "INDEL" ~ "FP.INDEL",
                                    QUERY == "FP" & QUERY.Var == "SNP" ~ "FP.SNP",
                                    QUERY == "." & TRUTH.Var == "INDEL" ~ "FN.INDEL",
                                    QUERY == "." & TRUTH.Var == "SNP" ~ "FN.SNP",
                                    TRUE ~ "other")) #%>% 
