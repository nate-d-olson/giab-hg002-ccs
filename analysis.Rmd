...
annotate_var_df <-  function(var_df){
    # target_cats <- c("lowcmp_SimpleRepeat_homopolymer_6to10", 
    #                  "lowcmp_SimpleRepeat_homopolymer_gt10",
    #                  "lowcmp_SimpleRepeat_imperfecthomopolymer_gt10_slop5",
    #                  "lowcmp_SimpleRepeat_diTR_51to200")
    target_cats <- c("lowcmp_AllRepeats_gt95identity_slop5")
    var_anno_df <- var_df %>% 
        mutate(target_cat = if_else(str_detect(Regions, target_cats),
                                    "target","non-target"))
...
var_random_df <- anno_df %>% 
    ...
    filter(var_cat != "AM" | target_cat != "non-target" | callset != "deep") %>% 
...
var_random_df <- anno_df %>% 
    ...
    dplyr::select(callset, CHROM, POS) %>% 
    left_join(anno_df) %>% 
    bind_rows(var_random_df)

deepvar_random_df <- var_random_df %>% 
    filter(callset == "deep") %>% 
    add_column(CHROM_POS = NA, .after = "callset") %>% 
...
var_region_heatmap_df <- var_random_df %>% 
    filter(callset == "deep") %>% 
    dplyr::select(variant, var_cat, Regions) %>% 
...
var_region_heatmap_df %>% 
    ...
    ggplot() + 
    geom_raster(aes(y = value, x = CHROM_POS, fill = var_cat)) + 
    theme(axis.text.x = element_text(angle = -90))
```
