...
<!-- TODO 
- Combine plots (multipanel?)
- Additional stratifications - map250 (proxy for LINES?), tandem repeats?
- fix bars starting from 1 
-->
...
```{r benchSummary, fig.cap = "Impact of re-genotyping using WhatsHap on GATK4 and DeepVariant callset benchmark metrics. Diff- all difficult regions, and Not Diff - not in difficult regions."}
## Do we have all difficult regions not in HG002allgenomespecific
high_level_strat <- c("*",
                       "alldifficultregions",
                       "notinalldifficultregions")
...
var_counts %>% 
  ...
  mutate(lci = qbinom(p = 0.025, size = count, prob = error_rate),
         uci = qbinom(p = 0.975, size = count, prob = error_rate)) %>% 
  select(-callset, -N, -Y) %>% 
